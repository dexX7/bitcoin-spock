...
class MetaDexSpec extends BaseRegTestSpec {
    ...
    def "Exact trade match: #amountMSC MSC for #amountSPX SPX"() {
        ...
        omniGetTrade(txidOfferA).propertyidforsale == propertyMSC.longValue()
        omniGetTrade(txidOfferA).propertyiddesired == propertySPX.longValue()
        ...
        omniGetTrade(txidOfferB).propertyidforsale == propertySPX.longValue()
        omniGetTrade(txidOfferB).propertyiddesired == propertyMSC.longValue()
    ...
    def "Exact trade match: #amountSPX SPX for #amountMSC MSC"() {
        ...
        omniGetTrade(txidOfferA).valid
        omniGetTrade(txidOfferA).status == "open"
        omniGetTrade(txidOfferA).propertyidforsale == propertySPX.longValue()
        omniGetTrade(txidOfferA).propertyiddesired == propertyMSC.longValue()
        ...
        omniGetTrade(txidOfferB).valid
        omniGetTrade(txidOfferB).status == "filled"
        omniGetTrade(txidOfferB).propertyidforsale == propertyMSC.longValue()
        omniGetTrade(txidOfferB).propertyiddesired == propertySPX.longValue()
    ...
    def setupSpec() {
        if (!commandExists("omniGetTrade")) {
            throw new AssumptionViolatedException('The client has no "omniGetTrade" command')
        }
        if (!commandExists("omniGetOrderbook")) {
            throw new AssumptionViolatedException('The client has no "omniGetOrderbook" command')
        }
    }
